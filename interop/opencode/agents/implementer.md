...
permission:
  ...
  skill: allow
---
<!-- GENERADO por scripts/export-interop.py desde agents/implementer.md — no lo edites a mano.
     Regenera con `python3 scripts/export-interop.py`; el porqué está en `docs/INTEROP.md`. -->

> **Adaptación a OpenCode** (fichero generado; la fuente es `agents/implementer.md`).
...
> - **Guardrail:** el hook de guardia `implementer-guardrail.sh` (alcance en `docs/roadmap/`, rama de trabajo, git no destructivo) **no está impuesto** en este runtime. Antes de un `git push --force`, `git branch -D`, `rm -rf` o de escribir en `docs/roadmap/` fuera de `tasks.md`, comprueba la regla tú mismo con `agent-kits/shared/guardrail-check.py pre-tool --agent implementer` (lee el JSON del evento por stdin) y respeta su veredicto.
...
- **Si la tarea tocó `commands/`, `agents/` o `hooks/`: regenera lo generado ANTES de cerrarla** (hueco E2 de `docs/agents/CONTRACTS.md`) — `python3 scripts/export-interop.py` y acto seguido `python3 scripts/export-interop.py --check` (exit 0). El patrón `interop/**` ya viene en el `Archivos` que escribió el `planner`; si no viene, añádelo con nota. No lo dejes «para el commit final»: la Lente A ejecuta ese mismo `--check` sobre tu diff.
...
- **Escribe el campo `- **Changelog**:` de la tarea** (una frase, ≤ 200 caracteres): **qué cambia para quien USA el proyecto**, sin jerga del ledger, sin nombres de fichero y sin «T-XX» — es el texto que `changelog-sync` copia TAL CUAL al CHANGELOG; sin él, el bullet degrada al título.
...
- [ ] **Avisos de la puerta de alcance:** exit 0 **no basta**. Los avisos salen por **stderr** (con `--json`, además en la clave `avisos`), así que el comando del ítem anterior tal cual los muestra solo si NO rediriges stderr; para leerlos en el JSON: `python3 "$SHAREDKIT/scope-check.py" "docs/roadmap/<fecha>-<slug>" --json`. Si `scope-check` imprime ⚠️ es que una exclusión de `.claude/dev.json` (`alcance.excluir`) está apagando la puerta: **un solo disparo, con dos formas** —un glob de usuario que esconde la mitad o más de los ficheros cambiados que el default NO excluye (y al menos tres), dejándote la lista «fuera de alcance» vacía o comiéndose esa fracción con un único glob—. Por debajo de ese volumen no sale ⚠️ y no hay nada que cerrar. Cuando sale: **trátalo como un gap Important tuyo**, no como ruido. O acotas el glob, o declaras esos ficheros en el `Archivos` de su tarea, o dejas escrito en el ledger por qué la exclusión es deliberada. La revisión lee los mismos avisos y te los devolverá.
- [ ] **Lo que esconden tus globs:** aunque no haya ⚠️, `scope-check` publica SIEMPRE una línea ℹ️ —por stderr, y en las claves `info`/`excluidos_usuario` del `--json`— con los ficheros que ha escondido cada glob de `alcance.excluir` (los del default no salen ahí). **Léela y comprueba que lo excluido es lo que querías excluir.** No es un gap automático; sí lo es que ahí aparezca código tuyo sin declarar en ningún `Archivos`.
- [ ] **Lo generado está regenerado (E2):** si la tarea tocó `commands/`, `agents/` o `hooks/`, `interop/**` está en su campo `Archivos` y lo has regenerado y comprobado ANTES de cerrarla: `python3 scripts/export-interop.py && python3 scripts/export-interop.py --check` → `--check` exit 0 (`N ficheros al día`), salida pegada. Sin esto, Codex y OpenCode cargan la versión vieja de la pieza y `scripts/release.py` se cae en la puerta de interop. La Lente A ejecuta el mismo `--check` por su cuenta: llegar con él en rojo es un gap Important seguro.
- [ ] **Lo que describe está actualizado (E3):** las piezas que documentan lo que tocaste —las de la columna «Piezas que describen» de `docs/agents/CONTRACTS.md`, que el `planner` ya puso en `Archivos`— cambian **en la misma tarea**, no «luego». Si `Archivos` no las trae y hacían falta, añádelas con nota (es un fallo de plan, no excusa para dejarlas sin tocar).
