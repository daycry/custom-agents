...
- **Memoria técnica del proyecto — lectura (siempre activa, D3).** Antes de descomponer (P2/P3), aplica el paso compartido `"$SHAREDKIT/knowledge-check.md"`: si existe `docs/knowledge/`, lee su `README.md` y abre las entradas de `adr/` + `lessons/` que apliquen (decisiones y lecciones de proceso que puedan condicionar el diseño del plan) y, si `docs/knowledge/journal/` tiene una última entrada de ESTA iniciativa, solo esa (pendientes que el plan debe recoger). Si no existe, continúa sin ella. Fallback si el fragmento no está: sigue sin este paso, no bloquea.
