...
| **confluence-publish** | Publica/espeja la doc del proyecto en Confluence vía el conector Atlassian (Rovo MCP), con **política curada** (opt-out sobre `include: ["**/*.md"]`, ver su `SKILL.md` sección "qué sube y qué no") y **staging generado** (`confluence-scope.py --stage` regenera `docs/confluence/`). Cada proyecto elige espacio y anclaje (raíz o hijo del árbol) en `.claude/confluence.json`; idempotente (crea/actualiza). | planner, evaluator, documenter, implementer, `/retro`, `/spec-drift`, `/roadmap-brief` |
...
| **adversarial-review** | Revisión adversarial de un diff con lentes de contexto fresco en paralelo: A (conformidad con spec/plan/constitución, ✓/✗ por criterio), B (solo defectos de corrección) y C (seguridad, **condicional**: la decide `review-lens-select.py` por rutas/líneas sensibles del diff; `dev.json` `revision.lenteSeguridad`). Fusión, graduación Critical/Important/Minor, bucle acotado a 3 con rebate por evidencia, traza «Revisión de dos lentes — intento N» en el ledger. **Fuente única del método**; también a demanda sobre una rama sin ledger. | `/dev-cycle` (Fase 3), `quick-implement`; a demanda («revísame este diff») |
...
└── docs/                    # TODA la documentación (estás aquí)
    ...
    └── knowledge/           # memoria técnica: adr/ · gotchas/ · lessons/ · README.md (índice)
```
