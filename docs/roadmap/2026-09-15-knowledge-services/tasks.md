...
| 24 | Important | La Verificación de T-02 declara el gate de CI completo pero su «salida real» remite a una nota que no existe en el ledger: ✓ sin evidencia (`verificacion: obligatoria`) | T-02 | pendiente | Lente A (`tasks.md:61,63`) |
| 25 | Important | El parser de listas en bloque (gap 9) solo reconoce items INDENTADOS: `enlaces:` + `- GOT-999` (YAML valido, salida por defecto de PyYAML) deja la clave `""` -> enlaces rotos no detectados (no-op silencioso) y `fuentes`/`tags` en bloque sin sangria disparan un falso positivo de la validacion nueva del gap 3 (`knowledge-index.py:129-150,188-196`) | T-02 | pendiente | Lente B, probe |
| 26 | Important | Lectura de entradas con `utf-8-sig` captura solo `OSError`: un `.md` en cp1252/latin-1 lanza `UnicodeDecodeError` fuera de `build_index` -> traceback y se pierde el indice entero (`knowledge-index.py:239-243`) | T-02 | pendiente | Lente B, probe |
| 27 | Important | `cargar_taxonomia()` conserva `except (OSError, JSONDecodeError)`: `taxonomy.json` en UTF-16 -> `UnicodeDecodeError` sin capturar; los gaps 14/15 taparon ese agujero en `default_taxonomy` y `main` pero no en el lector del fichero del usuario (`knowledge-schema.py:309-313`). Y con BOM UTF-8 (VS Code «UTF-8 with BOM», Notepad) se rechaza como «JSON ilegible» pese a ser valido: Knowledge Gate en rojo y `/doctor` en error en la plataforma principal (`:310`) | T-01 | pendiente | Lente B, probes (fusiona B3 + B4) |
| 28 | Important | Los tres scripts (`knowledge-schema.py:36-41`, `knowledge-index.py:47-52`, `capabilities.py:27-32`) no llevan el snippet de consola de la regla 8 de CONVENTIONS en su forma literal (`for _s in (sys.stdin, sys.stdout, sys.stderr): try: _s.reconfigure(encoding="utf-8", errors="replace") …` como en `model-tier.py:38-40`) ni entrada en `MODOS` de `tests/test_console_encoding.py`: `15 failed, 330 passed`; falla igual en Linux/CI. Precede a la ronda fix2 (nacio en T-01/T-02/T-13) | T-01/T-02/T-13 | pendiente | Lente B + orquestador (`pytest tests/test_console_encoding.py`) |
| 29 | Minor | Cola del gap 5: `cargar_taxonomia()` sin `root` devuelve `id_prefix` `"ca"` en vez del slug del cwd, aunque el gap 12 ya resuelve `root=None` -> `"."` en la misma funcion (`knowledge-schema.py:176` vs `:307`) | T-01 | pendiente | Lente A, probe |
| 30 | Minor | `os.walk` (gap 17) visita dos veces un fichero si la taxonomia declara `folder: "adr"` y `folder: "adr/legacy"` (config legal segun `_folder_seguro`) -> «id duplicado» falso contra el MISMO fichero, exit 1 (`knowledge-index.py:222-238`) | T-02 | pendiente | Lente B, probe |
| 31 | Minor | `_ruta_segura_dentro` usa `os.path.commonpath`, que lanza `ValueError: Paths don't have the same drive` con un symlink/junction a otra unidad: la defensa CWE-22 revienta con traceback en vez de degradar (`knowledge-index.py:170-172`; el test de symlink sale `SKIPPED` en Windows: confirmar en Linux/CI) | T-02 | pendiente | Lente B, probe |
| 32 | Minor | `_CACHE_TAXONOMIA` solo se vacia dentro de `enumerar()`; `evaluar_capacidad()` (API publica) deja cache residual y dos llamadas directas sirven la taxonomia obsoleta (`capabilities.py:111-136`) | T-13 | pendiente | Lente B, espia |
| 33 | Minor | Una clave sin valor (`estado:`) produce `""` y la validacion del gap 3 la reporta como «declarado (``) no es valido», mensaje engañoso para «campo presente sin valor» (`knowledge-index.py:149,182-187`) | T-02 | pendiente | Lente B |
| 34 | Minor | Vocabulario de `estado` bajo `approved/`: el codigo exige `"aprobado"` (`knowledge-index.py:175`); las carpetas son inglesas (`approved/`, `pending`, `needs_changes`, `rejected`) y la spec no fija el token. Coherente con la regla del plugin (tokens parseados en español) pero debe quedar ESCRITO en el esquema/plantilla y en `docs/knowledge/approved/README.md` para que `knowledge-curator` (T-04) use el mismo | T-02/T-04 | pendiente | Lente B (fuera de lente) + orquestador |
