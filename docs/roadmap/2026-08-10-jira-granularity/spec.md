...
| Tiempo acumulado | **El worklog `[revisión]` acumula TODAS las pasadas del reviewer, imputadas UNA POR INTENTO** (`--attempt N`: cada pasada es su propia entrada de worklog con duración y fecha; `reviewAttempts` en jira-state guarda la traza para /retro); las correcciones del implementer suman a su entrada `[implementación]`. Ambas al total del issue | Es tiempo real; el desglose separa implementación vs revisión Y se ve cuánto costó cada vuelta. Sin cronómetro: horas estimadas/reales por bloque, como el resto del plugin |
...
| C-07 | Worklog de revisión (entrada `[revisión]`, ambos modos, acumula todas las pasadas) | Imputar el tiempo del revisor como **entrada de worklog separada** marcada `[revisión]`, distinta de la de implementación, en **ambos modos** (issue de fase / issue de tarea según granularidad). **Acumula todas las pasadas del bucle, imputadas una por intento** (`--attempt N` → entrada de worklog por pasada + registro `reviewAttempts: [{intento, fecha, horas}]` en jira-state para /retro); las correcciones del implementer suman a la entrada `[implementación]`. `worklog.py` gana un tipo/etiqueta de entrada; `jira-state.json` guarda el desglose implementación vs revisión. El total del issue en Jira = implementación + revisión; respeta el tope de jornada y el banco igual que las demás entradas |
...
7. El tiempo de los reintentos se imputa **una entrada por intento** (traza de cuánto costó cada vuelta, con su fecha), con registro `reviewAttempts` para `/retro`; no hay cronómetro start/stop — horas por bloque como el resto del plugin. **Confirmado** (2026-08-10).
