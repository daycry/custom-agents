...
- **Tiempo IA**: real 1.65h (medido; usage-meter, T-01 0.14h + T-01-fix1 0.36h + T-01-fix2 1.15h, artefactos `graphiti-memory/T-01`, `graphiti-memory/T-01-fix1` y `graphiti-memory/T-01-fix2`)
...
- **Tiempo humano**: est. - · real -
- **Tiempo IA**: real 1.62h (medido; usage-meter, T-02 0.33h + T-02-fix1 0.14h + T-02-fix2 1.15h, artefactos `graphiti-memory/T-02`, `graphiti-memory/T-02-fix1` y `graphiti-memory/T-02-fix2`)
**Criterios de aceptación**
...
| # | Grado | Gap | Tarea | Corrección | Evidencia |
|---|---|---|---|---|---|
| 27 | Minor | `timeout_ms: true`/`health.timeout_ms: true` se rechazan bien pero ningún test lo cubre: quitar `isinstance(valor, bool)` deja la suite en verde (`knowledge-schema.py:205`) | T-01 | pendiente (Fase 2, paso previo) | mutante M18 vivo |
| 28 | Minor | Colisión mixta explícita↔implícita no detectada en la propuesta: `entity_map: {"ADR": "Decision"}` + categoría `decision` sin mapeo → una sola entrada `Decision` para dos categorías (`graphiti_model.py:204-215`); el chequeo de #18 solo compara derivado-vs-derivado | T-02 | pendiente (Fase 2, paso previo) | probe |
...
| 30 | Minor | El mensaje «`timeout_ms` debe ser numérico, finito y mayor que 0» no dice «entero»: `3000.0` se rechaza con un mensaje que el valor cumple (`knowledge-schema.py:310,383`; CONVENTIONS dice «entero») | T-01 | pendiente (Fase 2, paso previo) | probe |
