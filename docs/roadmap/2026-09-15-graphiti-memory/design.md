...
## Límites conocidos de la puerta estática de hooks (deuda declarada, 2026-09-23)

La revisión de la Fase 4 (intento 3) cerró el bucle con 0 Critical / 0 Important. La micro-ronda `fix3` corrigió en código los tres Minor baratos (#186: el comentario de shell se quita línea a línea; #190(d): test de la valla sin cerrar; #194: `--ia on` buscado por línea lógica). Los que siguen son **límites conocidos** de una puerta ESTÁTICA (AST + tokens) en `tests/test_graphiti_security.py`, no evasiones reales: ningún script alcanzable real los usa hoy, y la suite del repo los pinaría al primer cambio que los introdujera en `hooks/` o en lo que los hooks alcanzan, porque la puerta se ejecuta sobre el árbol REAL en cada CI.

- **Excepción del journal por fichero y motivo, no por sitio de llamada (#187).** `_EGRESS_OPT_IN_DECLARADO` (`tests/test_graphiti_security.py:156`) se aplica en `:524-526` retirando los motivos `ejecutable claude`/`argv <variable> -p` de TODO `journal.py`, y `_egress_del_journal_sin_condicion` (`:316-340`) mira `ia_activa` por subcadena (`:323`): una segunda función de IA sin guardia en el mismo fichero (X11) o `is True or bool(…)` (X10) pasan. Mitigación actual: `journal.py` tiene hoy una única llamada (`resumen_ia`) y la guardia se comprueba en cada llamada a esa función; un cambio en `journal.py` pasa por revisión adversarial con esta sección como contexto.
- **AST solo primer literal y listas incompletas (#188).** `_egress_por_ast` (`:275-313`) reconoce el binario de red solo como PRIMER literal de la lista (`["timeout","5","ssh",h]`, `["env","curl",url]`, `exe = shutil.which("curl")`, `f"ssh {h}"`, `Popen(args=…)`, `create_subprocess_shell(c)` pasan), y `_BINARIOS_DE_EGRESS`/`_SHELLS`/`_MODULOS_RED`/`_EXT_NO_ESCANEABLES` (`:95-107`, `:145-147`) no incluyen `ruby`, `.cjs`, `node -e`, `python3 -c`, `curl.exe`, `powershell irm`, `asyncio.open_connection`, `smtplib` ni `ctypes`. Mitigación actual: ningún script alcanzable real usa hoy ninguna de esas formas; la suite del repo lo pinaría al añadir cualquiera de los binarios o módulos ya listados.
- **Falso positivo del bloque como unidad (#189).** En `_ofensores_de_sincronizador` (`:1581-1600`, rama del bloque `:1592-1598`) un bloque ```bash``` con un `--check` legítimo y además un `git apply fix.patch` o `kubectl apply` se lee como publicación (el token `apply` de `_invocacion_de_escritura`, `:1513-1531`). Mitigación actual: ninguna pieza real lo hace hoy (el repo pasa en verde); si aparece, la suite lo pinaría en rojo —el fallo es seguro (falso positivo, no falso negativo)—.
- **Tokenización de la puerta CA-05 (#190 a/b/c/e).** (a) `--backend=graphiti` no se separa en `_tokens` (`:1488-1493`) y `_invocacion_de_escritura` (`:1519`) no ve `--backend`; (b) un `--check` dentro de un comentario final cuenta como flag de solo lectura (`:1525-1531`); (c) `_backend_de` (`:1442-1444`) toma el PRIMER `--backend` y argparse el último, así que `--backend kwipu --backend graphiti` queda exenta en las piezas de `_exenta`; (e) `_valor_de_variable_shell` (`:368-376`, `re.search` en `:372`) usa la PRIMERA asignación de la variable. Mitigación actual: ninguna pieza real usa hoy la forma `=`, dos `--backend` ni reasigna la variable del script; la suite del repo lo pinaría al primer `--backend graphiti` con escritura en la forma separada.
- **`_CITAS_POR_NOMBRE` sin test de uso (#191).** `test_f4fix2_las_citas_por_nombre_del_repo_estan_declaradas_una_a_una` (`:853-868`) comprueba que citante y destino existen y coincide el basename (tabla en `:131-136`), no que la entrada se USE en el recorrido ni que el destino sea el directorio real que resolvería el runtime. Mitigación actual: la tabla tiene hoy 4 entradas revisadas a mano; una entrada obsoleta solo deja de ampliar el recorrido, y `test_el_recorrido_alcanza_de_verdad_los_scripts_de_los_hooks` (`:551`) pinaría la pérdida de un script esperado.
- **`.sh` sin trocear por palabras (#192).** Para ficheros alcanzables, `_ofensores_de_red` (`:519-523`) aplica a los `.sh` solo las subcadenas de `_TERMINOS_RED` (`:88-92`); el troceo por TOKEN de `_BINARIOS_DE_RED` es solo para cadenas inline (`:520-521`), así que `nc a.tld 80`, `ssh a.tld id`, `C=$(command -v curl); "$C" …` o `curl<TAB>"$u"` en un `.sh` pasan. Mitigación actual: los `.sh` alcanzables reales (`hooks/*.sh`) no usan hoy ningún binario de red; la suite del repo lo pinaría con `curl `/`wget ` separados por espacio, la forma habitual.

Iniciativa futura propuesta: **`hooks-gate-hardening`** — llevar la puerta estática de hooks a un análisis por sitio de llamada (AST de flujo simple + tokenización completa de `.sh` y de la CLI CA-05), cerrando #187-#192.
