...
| `knowledge-services/training.json` | Config opt-in de `training-data-services`: `version` (1), `enabled` (default `false`), `root` (raíz del case store que elige el proyecto, relativa a su raíz o absoluta, sin `~`; nunca dentro de `<proyecto>/docs/knowledge/`, resuelto con `realpath` y sin distinguir mayúsculas, ADR-019), `id_prefix` (slug que prefija `case_id` = `<id_prefix>-<family>.<variant>`), `ids` (`family_pattern`/`variant_pattern`/`version_width`, opcional) y `bridge_to_curator` (default `false`: un caso Gold puede proponerse como candidato a `knowledge-curator`, nunca aprobado solo). Validado con `skills/training-data-services/scripts/case_schema.py` (stdlib; clave desconocida → error). Sin este fichero o con `enabled: false`, la capacidad `training` no existe para el ciclo (CA-01). Los casos y el dataset viven en `root`, **no** en `.claude/` ni en Git | `/setup` (paso de la capacidad `training`) o a mano | Recrear a mano (cinco claves); perderlo solo apaga la capacidad: el case store en `root` no se toca |
...
inválida → ❌ con fichero+campo+arreglo; desactivada → ℹ️; **desactivada y sin su `config_path`
en el proyecto → no pinta fila** —regla genérica del contrato: un opt-in que el proyecto no ha
tocado tiene impacto cero, sea `kwipu`, `training` o `graphiti`; `/doctor --verbose` (alias
`--all`) las lista igualmente, y una capacidad sin `config_path` declarado sale siempre—; activa con un backend declarado →
