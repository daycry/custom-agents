...
| Ctrl+C / closed terminal / process killed before the hook runs | Recovery up to the **last captured prompt** by `UserPromptSubmit` (the prompt log is the checkpoint): `journal.py recover` materializes it as `cierre: recuperado_sin_cierre` once the window passes (`sesion.journal.ventanaHuerfanaMin`, default 360 min). |
...
| `derivados_en` | `replay` (or `recover`): the fields that depend on `git` (`ficheros_tocados`, `tareas_cambiadas`) describe the repo's state AT MATERIALIZATION time, not at session close — CA-01 forbids running git in the teardown, so there is no other possible source. |

End-to-end measurement (CA-02): `python3 scripts/bench-session-end.py --iterations 30
--assert-p95-ms 100 --assert-p99-ms 300` measures the wall-clock time of `journal.py capture-end`
against a fixture project, the same path run by `hooks/session-journal.sh` in the teardown (without
`bash`'s startup, a fixed cost unrelated to the capturer). Exit 1 if a percentile exceeds the
requested threshold; `ci.yml.MANUAL-COPY` runs it with CA-02's thresholds on every build.
