...
| **`/dev-cycle <goal>`** | Development | Full cycle `evaluation → plan → implementation → tests → documentation` | **Native chain ALWAYS by default** (opt-in discipline in `.claude/dev.json`: TDD, worktrees, fresh subagents); superpowers only on explicit request (`--superpowers`). If started on a folder holding a spec+evaluation from `/pm-cycle`, it continues straight into planning. |
