...
porqué de cada fusión. Cubre los **10 agentes** de `agents/` más los **4 orquestadores** del plugin:
...
> `documenter` → `knowledge-curator` (Fase 4-bis, opcional según haya candidatos);
> `adversarial-review` secuencia 2-4 lentes ejecutadas por `reviewer` en paralelo,
...
| **documenter** | Agente | Estructura de `docs/` derivada del propio proyecto; qué categoría de contenido se omite y por qué (declarado, no silencioso); si al cerrar detecta un candidato de conocimiento, en qué categoría/evidencia lo propone. | El árbol de `docs/` (excepto `docs/roadmap/**` y `docs/security-scan/**`, que solo enlaza); una PROPUESTA por candidato en `docs/knowledge/candidates/pending/` (buzón — ver `knowledge-curator`). | Todo el repo; `docs/knowledge/**` completo (es quien lo indexa en la documentación de producto). | No implementa ni toca código, no escribe en `docs/roadmap/`, no decide el roadmap, **no aprueba ni mueve** un candidato a `approved/` (eso es `knowledge-curator`) — sin propuesta no hay fallo. |
| **knowledge-curator** | Agente | Qué categoría exacta le corresponde a un candidato; si su evidencia alcanza el `min_evidence` de esa categoría; `approved`/`needs_changes`/`rejected` por candidato (contrato via `curator-gate.py`, determinista); cuándo una contradicción o un alto impacto exige preguntar al usuario en vez de decidir solo. | `docs/knowledge/candidates/**` (mueve/actualiza) y `docs/knowledge/approved/**` (único escritor, `ADR-018`). | La taxonomía del proyecto (`.claude/knowledge-services/taxonomy.json` o el default del plugin), el índice de `approved/`. | No exporta a ningún backend (Kwipu/Graphiti, eso es `knowledge-sync.py`/T-07..T-09), no toca `docs/roadmap/` ni código, no aprueba con `errores != []` del gate. |
...
| **/dev-cycle** | Orquestador (comando) | Modo (vía rápida / completo), secuencia de agentes, cuándo relanzar revisión/qa, transiciones de estado del plan/`tasks.md`; en qué punto dispara cada evento de `jira-flow.py` (tabla única en `commands/dev-cycle.md`, T-03 de esta iniciativa); si hay candidatos de conocimiento que curar tras `documenter` (Fase 4-bis, `knowledge-services` T-06) — con omisión honesta si no hay ninguno, y sin que Kwipu condicione nunca el cierre. | Transiciones de estado (plan/tasks/spec); invoca los eventos Jira en los puntos que fija esta iniciativa. | Toda la carpeta de la iniciativa; `docs/knowledge/candidates/**` para decidir si hay algo que curar. | No escribe código ni documentación (delega en los agentes), no decide el veredicto de revisión/qa (lo dan sus scripts, `qa-gate.py`/la fusión de `adversarial-review`), no decide el veredicto del Knowledge Gate (lo da `curator-gate.py` vía `knowledge-curator`). |
| **adversarial-review** | Orquestador (skill) | Qué lentes aplican (A/B siempre; C seguridad y D rendimiento, condicionales vía `review-lens-select.py`); cómo se funden y gradúan (Critical/Important/Minor) las salidas de 2-4 `reviewer`. | La sección `## Revisión de dos lentes — intento N` en `tasks.md`; promueve entradas `docs/knowledge/` de `propuesta` → `aceptada` al cerrar sin gaps; **publica en Jira el evento `revision`/`gaps` de CADA intento** (`jira-flow.py`, firmado `ca-reviewer`; opt-in, degrada con aviso) — dueño único desde `jira-review-comments` (2026-09-19). | El diff, los artefactos de la iniciativa, la tabla del intento anterior. | No implementa ni corrige (eso es `implementer`), no da el verde de pruebas (`qa`), no imputa horas (es del orquestador `/dev-cycle`, vía `jira-flow.py`/`worklog.py`), no decide qué pasa al 3.er intento con gaps (lo devuelve). |
