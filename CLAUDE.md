...
├── hooks/                   # PostToolUse · SubagentStop · SessionStart · UserPromptSubmit (log crudo del turno, opt-out `<private>`) · SessionEnd (journal de sesión) — informan (systemMessage/additionalContext) o escriben la bitácora, no deciden; siempre exit 0
├── statusline/              # roadmap-statusline.sh (opt-in en /setup): modelo · coste sesión · ctx · progreso roadmap
├── scripts/                 # lint_plugin.py · release.py (release mecánico COMPLETO: changelog + checks + bump + chmod + commit/tag) · export-skills.py (paquete portable «solo skills» → dist/, ignorado) · export-interop.py (traduce las piezas a Codex/OpenCode; determinista, `--check`)
├── install/                 # instalador `npx` multi-proveedor (install.mjs + providers.mjs; cero dependencias, idempotente, `uninstall` por manifiesto)
├── interop/                 # GENERADO por export-interop.py: interop/codex/ (agentes .toml, prompts, hooks) · interop/opencode/ (agentes, comandos, opencode.json, adaptador JS)
...
| Linter + tests | `python scripts/lint_plugin.py` (frontmatter, grafo `dependencies` sin ciclos, colisiones, `hooks/hooks.json` con commands existentes y ejecutables; avisa si una pieza no tiene caso positivo en `evals/cases/` o su description pasa de 1.200 caracteres; y avisa —hoy **aviso**, error tras una release limpia— si una pieza de `agents/`, `commands/`, `skills/` o `docs/` (menos el registro: roadmap, journal y ejemplos) **cita** una ruta del repo o un `/comando` que no existe, o si una fila de `docs/agents/CONTRACTS.md` se queda sin Puerta) + `python evals/check.py` (toda pieza con ≥ 2 positivos + 1 negativo y el literal casando con la description REAL) + suites de `tests/` antes de publicar. Opcional con secret: `headless.yml` (evals reales + hooks en sesión real). **Publicar = `python3 scripts/release.py X.Y.Z`** (nunca a mano): mueve `[Unreleased]`/`[Sin publicar]` a la sección de la versión en los dos CHANGELOG, corre lint + evals, exige copias `.MANUAL-COPY` al día, corrige `.sh` en `100644`, bump en los 5 manifiestos (los 2 de Claude Code, los 2 de Codex y `package.json`), comprueba `export-interop.py --check`, commit + tag; `--dry-run` antes, `--check` después (LES-012). |
...
| Interop (Codex · OpenCode) | Las piezas reales (`agents/`, `commands/`, `skills/`, `hooks/`) son la ÚNICA fuente; lo de `interop/`, `.codex-plugin/` y `.agents/plugins/` lo **genera** `scripts/export-interop.py` y no se edita a mano. **Al tocar un agente, comando o hook: regenera** (`--check` es puerta de CI y de `release.py`). Las skills viajan sin traducir, pero su `description` **no puede pasar de 1.024 caracteres** (por encima, OpenCode no carga la skill: lo avisa el linter y lo falla `tests/test_export_interop.py`). El `find` de la regla 5 lleva **seis raíces** (`.claude`/`.codex`/`.opencode`, proyecto y usuario). Degradación por runtime y qué se pierde: `docs/INTEROP.md`. |
...
| Memoria técnica | `docs/knowledge/` (siempre activa, sin opt-in): `adr/ADR-NNN-<slug>.md`, `gotchas/GOT-NNN-<slug>.md`, `lessons/LES-NNN-<agente>-<slug>.md`, con `README.md` como única puerta de entrada (índice). Lectura selectiva por área (`agent-kits/shared/knowledge-check.md`: evaluator/planner/implementer/qa/documenter); escritura con umbral anti-burocracia (`agent-kits/shared/knowledge-write.md`: ADR solo si cierra alternativa y afecta 2+ piezas, gotcha solo si costó ≥1 ciclo de depuración). **Journal de sesión** (`docs/knowledge/journal/`, memoria EPISÓDICA no curada, ADR-010): el hook `SessionEnd` escribe una entrada determinista por sesión con `agent-kits/shared/journal.py` (idempotente por `session_id`; `dev.json` `sesion.journal: false` la apaga) y `SessionStart` (`startup\|resume`) reinyecta la última (≤ 25 líneas); excluido de Confluence; lo que merezca doctrina se promueve a ADR/gotcha/lección. Desde `memory-retrieval` (F4-F6, ADR-013): `UserPromptSubmit` acumula el turno del usuario en `.claude/session-prompts-<session_id>.log` (no versionado, `<private>` por turno, secretos evidentes redactados; `sesion.captura: false` lo apaga) y `SessionEnd` extrae de ahí `decisiones`/`pendientes` **sin modelo** (citas, no instrucciones); resumen por IA **opt-in** (`sesion.resumen: true`, `claude -p --bare`, degrada siempre) — ADR-010 revisado: la salida de los hooks en `SessionEnd` sigue ignorándose, pero el hook **escribe**. `journal.py candidatas` propone como `propuesta` los patrones repetidos en ≥ 2 sesiones (paso 2-quater de `/retro`). **Doctrina del plugin** (`agent-kits/evaluator/assets/doctrina/`, `knowledge-find.py --doctrina`): las 9 lecciones de estimación viajan con el plugin; la memoria del consumidor nace vacía. **La retro es puerta de cierre** (`agent-kits/shared/retro-gate.py`, `/dev-cycle` Fase 6 paso 8). |
...
| **documenter** | Documentación técnica/producto del proyecto bajo `docs/`, derivada del repo. Una vez al cierre del ciclo. No toca `docs/roadmap/` ni `docs/security-scan/`. Al cerrar, **propone** (nunca aprueba) candidatos de conocimiento en `docs/knowledge/candidates/pending/`. |
| **knowledge-curator** | Único agente que aprueba/rechaza/pide cambios sobre `docs/knowledge/candidates/` y único que escribe en `docs/knowledge/approved/` (`ADR-018`). Valida categoría exacta, evidencia mínima, `fuentes`, `tags` y lista negra con `curator-gate.py` (determinista); pregunta al usuario ante contradicciones/alto impacto. No exporta a backends ni toca `docs/roadmap/`. |
...
| `dependency-upgrade` | Inventario de dependencias sin ejecutar nada (`scripts/deps-inventory.py`, 11 tests): 7 manifiestos + lockfiles, declarada/bloqueada/`latest` (solo del `outdated` oficial, nunca inventado), salto patch/minor/major; changelog upstream por major → `spec.md` «upgrade-…» con la plantilla del evaluator. Vulnerabilidades → nemesis. (a demanda; handoff evaluator → planner) |
| `changelog-sync` | Entradas `[Unreleased]` (EN) / `[Sin publicar]` (ES) de los DOS CHANGELOG derivadas de los ledgers **cerrados** (`estado: completado`): un bullet por `T-XX` de **una o dos frases** (escalera: campo `- **Changelog**:` de la tarea → 1.ª frase de la `Descripción` si cabe en 200 caracteres → oración principal → solo el título + puntero al ledger; sin truncar con `…`), hasta 3 ficheros, categoría `Added/Changed/Fixed` deducida o declarada (`changelog:`), idempotente por slug. No abre la sección de versión (eso es `release.py`) ni inventa alcance. (`/dev-cycle` Fase 6, `quick-implement`, precondición de `release.py`; a demanda) |
...
| `knowledge-services` | Publica `docs/knowledge/approved/` a un backend externo por proyecto (`taxonomy.json`), sin acoplar el plugin a ninguno: `knowledge-sync.py --backend <id>` carga un adaptador por `type` (contrato de 6 funciones), aplica `routing` fail-closed y usa `outbox.py` (staging/dead-letter). Adaptador Kwipu (`markdown-export`): Markdown + frontmatter del Knowledge Gate + `manifest.json`; `verify` nombra el reindexado sin ejecutarlo. (capacidad opt-in `kwipu` de `/setup`/`/doctor`; a demanda) |
| `training-data-services` | Captura determinista de casos (petición, contexto, trayectoria sin chain-of-thought, métricas opacas, validación) en un case store FUERA de Git y de `docs/knowledge/`, y dataset solo con Gold humano. Opt-in `.claude/knowledge-services/training.json`; `case_schema.py` valida config y caso (vocabularios cerrados, mapeo de `outcome` desde graphify). Sin dominio: no calcula métricas, no marca Gold, no entrena. (capacidad opt-in `training`; a demanda) |
