...
- **T-01 — El README dice lo que el plugin hace, no contra quién** El README ya no compara el plugin con otros: describe lo que hace y deja de ofrecer delegar la implementación en un motor externo. (`README.md`, `README.es.md`)
- **T-02 — `/dev-cycle` retira el Modo A: la cadena nativa es el único motor** `/dev-cycle` ya no tiene modo de motor externo ni flag `--superpowers`: la cadena nativa del plugin es el único motor de implementación, y el comando queda más corto y con una sola ruta.
- **T-03 — Las notas de atribución describen el patrón, no la fuente** Las notas de diseño del plugin explican el patrón que aplican en vez de citar de qué plugin viene, sin perder nada de lo que decían técnicamente.
- **T-04 — El fixture del test de `skill-index` deja de citar un flag inexistente (y el recorte pasa a estar probado)** El índice de piezas que se inyecta al arrancar sesión tiene ahora probado el recorte del `argument-hint` largo, que antes no lo estaba. (`agent-kits/shared/test_skill_index.py`)
- **T-05 — Las cifras VIVAS de la escalera del CHANGELOG se re-miden al entrar este ledger en el corpus** Las cifras medidas que la documentación del CHANGELOG afirma vuelven a reproducir con la medición del repo de hoy. (`skills/changelog-sync/references/medicion-escalera.md`, `skills/changelog-sync/SKILL.md`, `docs/knowledge/adr/ADR-012-resumen-del-changelog-lo-escribe-quien-cierra-la-tarea.md`)

### Added — iniciativa `memory-retrieval` (2026-09-04)

- **T-01 — Capa 1 de `knowledge-find.py`: consulta → aciertos compactos** Los agentes pueden consultar la memoria técnica del proyecto con una orden en vez de leer el índice entero: cada acierto es una línea compacta con el estado delante, y el área se casa sin acentos ni mayúsculas. (`agent-kits/shared/knowledge-find.py`, `tests/test_knowledge_find.py`, `tests/test_console_encoding.py`)
- **T-02 — Capa 2: `--related <ID>` con grafo curado** Desde cualquier entrada de la memoria técnica se puede pedir su grafo curado (qué la sustituyó o a qué sustituye, qué más salió de la misma iniciativa y qué comparte área), en vez de una lista cronológica. (`agent-kits/shared/knowledge-find.py`, `tests/test_knowledge_find.py`)
- **T-03 — Capa 3 (`--show`) e índice SQLite FTS5 reconstruible** La memoria técnica se puede abrir entera por su ID, y las consultas van sobre un índice de texto completo en `.claude/` que se reconstruye solo cuando el corpus cambia; si el índice no se puede usar, la consulta responde igual leyendo los ficheros. (`agent-kits/shared/knowledge-find.py`, `tests/test_knowledge_find.py`, `.gitignore`)
- **T-04 — Lint del índice: biyección `ficheros ↔ filas` y `area` obligatoria** El índice de la memoria técnica queda vigilado: una entrada sin fila, una fila sin área o un enlace roto hacen fallar el linter del plugin y la suite, nombrando el fichero o el ID. (`tests/test_knowledge_index.py`, `scripts/lint_plugin.py`, `tests/test_lint_plugin.py`)
- **T-05 — Memoria técnica en `task-brief.py`, presupuestada y enrutada por `Tipo`** El brief que recibe cada subagente trae ahora los aciertos de la memoria técnica que tocan su tarea (por tipo, título e iniciativa), con tope de 600 tokens y el estado de cada entrada delante; sin memoria en el proyecto el brief sale como antes. (`agent-kits/shared/task-brief.py`, `agent-kits/shared/test_task_brief.py`, `agent-kits/shared/knowledge-find.py`)
- **T-06 — Memoria del área activa al arrancar sesión, con tope propio** Al arrancar o retomar una sesión (también tras compactar), el contexto trae los aciertos de la memoria técnica del área de la iniciativa activa, topados a 300 tokens y sin desplazar el índice de piezas ni el roadmap; se apaga con `sesion.memoria: false`. (`hooks/session-context.sh`, `tests/test_hooks_shell.py`, `agent-kits/shared/doctor.py`)
- **T-07 — El reparto por agente de `knowledge-check.md` deja de ser solo prosa** Cada agente sabe ya qué orden exacta de la memoria técnica le toca ejecutar (por área, tipo de entrada o tarea) en vez de leer el índice entero y decidir; el revisor adversarial también consulta los ADR del área del diff.
- **T-08 — `tests/test_memory_path.py`: el camino se recorre, con su mutante** La suite comprueba, sin gastar tokens, que la memoria técnica del área de una tarea llega de verdad al brief del subagente y al arranque de sesión, y que las de otras áreas no: si alguien quita la inyección, la suite se pone roja. (`tests/test_memory_path.py`)
- **T-09 — Casos en `evals/` para el camino de memoria** La suite de evals comprueba que el implementador cita el gotcha de su área y el evaluador la lección de estimación que le toca, y no al revés, sobre un proyecto de prueba con memoria técnica inventada. (`evals/cases/agent-implementer.json`, `evals/cases/agent-evaluator.json`, `evals/fixtures/project/`)
- **T-10 — `/doctor` puntúa la salud de la memoria** `/doctor` deja de dar por sana una instalación con la memoria a medias: cuenta las entradas curadas por familia y estado, valida el índice de la memoria (❌ si una entrada no tiene fila), avisa si el journal sigue a 0 con memoria curada y si CALIBRATION.md lleva más de 14 días sin fila con iniciativas cerradas después, diciendo cuántas y cuáles. (`agent-kits/shared/doctor.py`, `agent-kits/shared/test_doctor.py`)
- **T-19 — Cierre de los gaps del intento 1 (Fases 1-3)** La consulta libre de la memoria técnica ignora las palabras sin contenido y ordena por título, área e ID antes que por el cuerpo: una pregunta en lenguaje natural trae lo que toca y una sin sentido no trae nada. El brief del subagente lleva la verificación una vez, sin rojos anteriores ni presupuesto, y cabe en 2.500 tokens; el arranque de sesión con varias iniciativas activas cuenta los aciertos una vez; `/doctor` valida el índice de memoria con el criterio exacto del linter.
- **T-11 — Hook `UserPromptSubmit`: log crudo no versionado con opt-out** Cada turno del usuario queda en un log crudo no versionado (`.claude/session-prompts-*.log`) para la bitácora, con opt-out por turno `<private>` y por proyecto, y sin frenar nunca la sesión. (`hooks/user-prompt-capture.sh`, `hooks/hooks.json`, `tests/test_hooks_shell.py`)
- **T-12 — `SessionEnd` escribe él mismo `decisiones` y `pendientes`; `ADR-010` revisado** La entrada del journal trae `decisiones` y `pendientes` reales, extraídas sin modelo de los turnos del usuario; `ADR-010` queda revisado: el hook no devuelve, escribe. (`agent-kits/shared/journal.py`, `hooks/session-journal.sh`, `agent-kits/shared/test_journal.py`)
- **T-13 — Resumen episódico por IA, opt-in y degradando siempre** Resumen de sesión por IA opt-in (`sesion.resumen: true`, `claude -p --bare`) que siempre degrada a la entrada determinista si no hay CLI, clave o respuesta válida. (`hooks/session-journal.sh`, `agent-kits/shared/journal.py`, `agent-kits/shared/test_journal.py`)
- **T-14 — Promoción journal → candidata a lección por la puerta de `/retro`** `journal.py candidatas` propone como lección `propuesta` los patrones que se repiten en dos o más sesiones; `/retro` los muestra y solo el usuario los promueve. (`agent-kits/shared/journal.py`, `commands/retro.md`, `agent-kits/shared/test_journal.py`)
- **T-20 — Cierre de los gaps del intento 1 (Fase 4)** Privacidad del journal de punta a punta: `<private>` ya no se filtra por la transcripción, los secretos evidentes se redactan, el log nace ignorado por git y 0600, y la entrada se escribe atómica. (`agent-kits/shared/journal.py`, `agent-kits/shared/test_journal.py`, `hooks/user-prompt-capture.sh`)
- **T-15 — Doctrina del plugin como assets, separada de la memoria del proyecto** Las 9 lecciones de estimación viajan con el plugin como doctrina del `evaluator` (copias byte a byte con test); la memoria del proyecto consumidor sigue naciendo vacía. (`agent-kits/evaluator/assets/doctrina/`, `scripts/export-skills.py`, `tests/test_doctrina_viaja.py`)
- **T-16 — El `evaluator` estima con ese fondo sin engordar su prompt** `knowledge-find.py --doctrina` lee la doctrina del plugin sin tocar la memoria del proyecto (cada acierto dice de dónde viene) y el prompt del `evaluator` adelgaza a 15.024 bytes. (`agent-kits/shared/knowledge-find.py`, `agents/evaluator.md`, `tests/test_doctrina_viaja.py`)
- **T-17 — `/retro` se dispara al cerrar una iniciativa** La retro es una puerta del cierre de `/dev-cycle` (`retro-gate.py`: `retro.md` + fila en `CALIBRATION.md`, exit 0 obligatorio), no un comando que alguien recuerda. (`commands/retro.md`, `commands/dev-cycle.md`, `agent-kits/shared/doctor.py`)
- **T-18 — Doc ES/EN y las entradas de `docs/knowledge/` que salen de aquí** La doc ES/EN describe la captura del turno, el resumen IA opt-in, la doctrina y la retro como puerta; ADR-013 fija las tres capas de la memoria y LES-015 la lección de privacidad de F4.
- **T-21 — Cierre de los gaps del intento 1 (Fases 5-6)** La puerta de retro acepta el formato real de las retros y entra en la suite de codificación; el titular de la doctrina sale del cuerpo; la spec pasa a `implementada` solo con la puerta abierta.
...
### Fixed — iniciativa `windows-console` (2026-09-03)

- **Todo script sobrevive a una consola que no es UTF-8** ([`GOT-005`](docs/knowledge/gotchas/)): en Windows con locale `cp1252`, `python scripts/release.py` moría con `UnicodeEncodeError: 'charmap' codec can't encode characters` — reportado con traceback real. Causa raíz: el crash ocurre en el **hijo canalizado**, donde Python cae al codepage ANSI del locale, no en la consola de PowerShell. Los 28 scripts versionados que imprimen símbolos o leen `stdin` reconfiguran ahora `sys.stdin`/`stdout`/`stderr` a UTF-8 con `errors="replace"` al arrancar, y el lado **padre** también queda arreglado: 56 llamadas a `subprocess` capturan con `encoding` explícito, y los ocho `python3 -c` en línea de `hooks/` y `statusline/` llevan `PYTHONIOENCODING=utf-8:replace`. Sin modo ASCII: medido, `reconfigure` gana a `PYTHONIOENCODING`, así que la salida sale en UTF-8 y `errors="replace"` no llega a dispararse.
- **El guardarraíl de alcance deja de abrirse solo**: `guardrail-check.py` leía el payload del hook con el codec del locale y convertía el `UnicodeDecodeError` en un *allow* silencioso — bajo `cp1252`, un solo emoji en el contenido escrito desactivaba los guardarraíles de rama y de alcance del `implementer`. Ahora decodifica como UTF-8 y deniega como debe.
- **Un crash nunca se presenta como veredicto**: `release.py` decía `changelog-sync --check: PENDIENTE` cuando el check había reventado (el exit 1 es el mismo). Las tres precondiciones distinguen ahora `OK` · `FALLA (exit N)` · `ERROR al ejecutar` (traceback en stderr, o exit ∉ {0,1}), imprimen las tres últimas líneas de stderr y **bloquean** el release — mientras que una deuda de notas legítima sigue siendo un aviso que no bloquea.
- **La regla se vigila, no solo se escribe**: `lint_plugin.py` avisa de forma estructural (`ast`, así que un snippet metido dentro de `main()` o citado en un docstring ya no cuela) y `tests/test_console_encoding.py` ejecuta cada script bajo `PYTHONIOENCODING=cp1252` y `ascii`, descubriendo la lista dinámicamente para que un script nuevo que imprima símbolos falle hasta que lleve el snippet. Las tres pasadas de revisión adversarial quedan en el ledger de la iniciativa, incluidos los dos intentos en que se encontró que el propio guardarraíl tenía agujeros.
